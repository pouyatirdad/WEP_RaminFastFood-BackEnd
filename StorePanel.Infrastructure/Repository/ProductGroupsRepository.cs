...
{
    public interface IProductGroupsRepository : IBaseRepository<ProductGroup>
    {

        List<ProductGroup> GetProductGroupTable();
        List<ProductGroup> GetProductGroupTable(int id);
        ProductGroup GetProductGroup(int id);
        List<Feature> GetFeatures();
        List<Feature> GetProductGroupFeatures(int id);
        List<Brand> GetProductGroupBrands(int id);
        List<Brand> GetBrands();
        List<ProductGroup> GetProductGroups();
        ProductGroup AddNewProductGroup(int parentId, string title, List<int> brandIds, List<int> featureIds,string Image);
        Task<ProductGroup> UpdateProductGroup(int parentId, int productGroupId, string title, List<int> brandIds, List<int> featureIds);
    }

    public class ProductGroupsRepository : BaseRepository<ProductGroup>, IProductGroupsRepository
    {
        private readonly StorePanelDbContext _context;
        private readonly ILogRepository _logger;
        public ProductGroupsRepository(StorePanelDbContext context, ILogRepository logger) : base(context, logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public ProductGroup AddNewProductGroup(int parentId, string title, List<int> brandIds, List<int> featureIds,string Image)
        {
            var productGroup = new ProductGroup();

            productGroup.InsertDate = DateTime.Now;
            productGroup.InsertUser = GetCurrentUsersName();
        ...
        public async Task<ProductGroup> UpdateProductGroup(int parentId, int productGroupId, string title, List<int> brandIds, List<int> featureIds)
        {
            var productGroup = await GetById(productGroupId);
            productGroup.UpdateDate = DateTime.Now;
            productGroup.UpdateUser = GetCurrentUsersName();
